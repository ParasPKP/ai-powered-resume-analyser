...
const postToParent = (level: string, text: string, extra: unknown) => {
  try {
    if (isBackend() || !window.parent || window.parent === window) {
      const allowedLevels = ['debug', 'info', 'warn', 'error', 'log'] as const;
      const consoleLevel = allowedLevels.includes(level as (typeof allowedLevels)[number])
        ? (level as (typeof allowedLevels)[number])
        : 'log';
      console[consoleLevel](text, extra);
      return;
    }
